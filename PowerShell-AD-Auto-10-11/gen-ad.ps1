...
function Process-Leaver {
    ...
    $Name = $User.name.Trim()
    if ($null -ne $User.kerberoastable) {
        $SamAccountName = $Name.ToLower().TrimEnd('.')
    }
    else {
...
function Create-ADUser {
    ...
    if (-not (Get-ADUser -Filter "SamAccountName -eq '$SamAccountName'" -ErrorAction SilentlyContinue)) {
        ...
        if ($User.showPassword) {
            $Description = "Your default password is $Password"
        }
        ...
        if ($null -ne $User.kerberoastable -and $null -ne $User.kerberoastable.spn) {
            $SPN = $User.kerberoastable.spn
            setspn -A $SPN "$Domain\$SamAccountName"
        }    
...
if ($Undo) {
    ...
    foreach ($User in $config.users) {
        ...
        if ($null -ne $User.kerberoastable) {
             $SamAccountName = $Name.ToLower().TrimEnd('.')
             if ($null -ne $User.kerberoastable.spn) {
                 $SPN = $User.kerberoastable.spn
                 setspn -D $SPN "$Domain\$SamAccountName"
             }
        }
        else {
             $FirstName, $LastName = $Name -split ' ', 2
